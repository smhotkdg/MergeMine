...
public class CollectingAnimation : MonoBehaviour {
    ...
    public void Initialize(Transform destination, Transform parent, Vector3 localPosition, Vector3 localScale, PLAY_SOUND_MODE playSoundMode, EXPANSION_MODE expansionMode) {
	...
	IEnumerator CollectAnimation() {
		...
		while (t < _expansionDuration) {
            ...
            if (_expansionMode == EXPANSION_MODE.UPWARD)
            {
                transform.localPosition += Vector3.Scale(direction, new Vector3(1, speed, 1));
            } else {
                transform.localPosition += Vector3.Scale(direction, new Vector3(speed, speed, 1));
            }
		...
		while (t < 1.0f) {
			t += Time.deltaTime * _animationSpeed;
			transform.localPosition = Vector3.Lerp(pos, _itemDisplayerTransform.localPosition, t);
